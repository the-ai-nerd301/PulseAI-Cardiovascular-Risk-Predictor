/* =========================================================
   Pulse-AI — Scroll-driven frame sequence (performance metrics)
   ========================================================= */
(function () {
    const TOTAL_FRAMES = 300;
    const FRAME_PATH = 'images/performance-images/ezgif-frame-';
    const FRAME_EXT = '.jpg';
    const PAD_LENGTH = 3;

    const wrapper = document.getElementById('scroll-anim-wrapper-perf');
    const canvas = document.getElementById('frame-canvas-perf');
    const frameCtx = canvas ? canvas.getContext('2d') : null;
    const loadingEl = document.getElementById('scroll-anim-loading-perf');
    const loadingPctEl = document.getElementById('loading-pct-perf');
    const cueEl = document.getElementById('scroll-anim-cue-perf');
    const scrimEl = document.getElementById('scrim-perf');

    const overlays = {
        intro: document.getElementById('overlay-intro-perf'),
        outro: document.getElementById('overlay-outro-perf'),
        accuracy: document.getElementById('label-accuracy'),
        precision: document.getElementById('label-precision'),
        recall: document.getElementById('label-recall'),
    };

    if (!wrapper || !canvas || !frameCtx) {
        console.warn('Performance scroll animation: missing wrapper or canvas.');
        return;
    }
    ...
    function updateScrollAnim() {
        ...
        setOverlayOpacity(overlays.accuracy,  fadeWindow(progress, 0.25, 0.30, 0.45, 0.50));
        setOverlayOpacity(overlays.precision, fadeWindow(progress, 0.30, 0.35, 0.50, 0.55));
        setOverlayOpacity(overlays.recall,    fadeWindow(progress, 0.35, 0.40, 0.55, 0.60));
